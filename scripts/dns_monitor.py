...
def score_threat(domain: str, record_type: str, rcode: str) -> Dict[str, Any]:
    """
    ...
    # 1. Domain entropy — DGA domains tend to have high entropy
    labels = domain.split(".")
    if labels:
        main_label = labels[0]
    ...
    # 2. Domain length — unusually long subdomains can signal tunneling
    if len(domain) > 60:
        score += 25
...
def persist_event(event: Dict[str, Any], args) -> None:
    ...
    # Only insert when batch was flushed (batch variable exists)
    if "batch" in dir():
        return

    try:
        if len(MONGO_BATCH) == 0 and "batch" not in locals():
            return
    except Exception:
        pass

    # Simplified: insert individually if batch not ready, batch if full
    try:
        collection.insert_one(event)
    except Exception as exc:
        emit({
            "kind": "error",
            "message": f"MongoDB insert failed: {exc}",
        })
...
def build_event(
    ...
    threat = score_threat(domain or "unknown", normalized_type, normalized_rcode)
...
def parse_scapy_packets(args) -> int:
    try:
        from scapy.all import DNS, DNSQR, DNSRR, IP, IPv6, PcapReader, TCP, UDP, sniff
    ...
    def extract_answers(dns_layer) -> Tuple[List[str], Optional[int]]:
    ...
    try:
        ...
        while RUNNING and process.stdout:
            ...
            for idx in range(11, 15):
                if parts[idx]:
                    answers.extend(parts[idx].split(";"))
...
Examples:
  ...
  %(prog)s --mode live --interface eth0 --mongo-uri mongodb://localhost:27017
    ...
    parser.add_argument("--mode", choices=["live", "manual"], default="live",
                        help="Capture mode: live (real-time) or manual (PCAP file)")
...
if __name__ == "__main__":
    try:
        ...
        emit({
            "kind": "status",
            "status": "interrupted",
            "note": "DNS monitor interrupted by user.",
        })
        sys.exit(1)
    except Exception as exc:
        emit({
            "kind": "error",
            "message": f"Unexpected error in main: {exc}",
        })
        sys.exit(1)
