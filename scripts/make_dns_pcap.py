...
def export_packet_metadata(packets: List[Any], csv_path: Optional[Path], json_path: Optional[Path]) -> None:
    ...
    from scapy.all import DNS, IP, IPv6, TCP  # pyre-ignore
    ...
    for p in packets:
        ...
        event = {
            "timestamp": float(getattr(p, "time", 0)),
        ...
        with open(str(csv_path), 'w', encoding='utf-8', newline='') as f:
            if events:
                writer = csv.DictWriter(f, fieldnames=[
                    "timestamp", "source_ip", "dest_ip", "transport", "dns_id", 
                    "is_response", "domain", "qtype", "rcode", "answers"
                ])
                writer.writeheader()
                for e in events:
                    row = e.copy()
...
def build_query_packet(
    ...
    from scapy.all import DNS, DNSQR, Ether, IP, IPv6, TCP, UDP  # pyre-ignore
...
def build_response_packet(
    ...
    from scapy.all import DNS, DNSQR, DNSRR, Ether, IP, IPv6, TCP, UDP  # pyre-ignore
...
def build_nxdomain_packet(
    ...
    from scapy.all import DNS, DNSQR, Ether, IP, IPv6, UDP  # pyre-ignore
